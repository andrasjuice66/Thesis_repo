...
from avalanche.benchmarks import tensors_benchmark, dataset_benchmark
import ewc_data
...
is above 90% but the average accuracy on previous tasks is around 20%.
...
def main(args):
    ...
    elif args.scenario == "iris":
        (train_data, test_data) = ewc_data.get_iris(5)
        scenario = dataset_benchmark(
            train_data,
            test_data)

        model = SimpleMLP(num_classes=args.output_size, hidden_size=args.hs, input_size=args.input_size)
    ...
    eval_plugin = EvaluationPlugin(
        ...
        loggers=[interactive_logger, tensorboard_logger], benchmark=scenario
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scenario",
        type=str,
        choices=["pmnist", "smnist", "multitask_smnist", "iris"],
        default="iris",
    ...
    parser.add_argument("--hs", type=int, default=2, help="MLP hidden size.")
    parser.add_argument(
        "--epochs", type=int, default=10, help="Number of training epochs."
    )
    parser.add_argument(
        "--minibatch_size", type=int, default=50, help="Minibatch size."
    )
    parser.add_argument(
        "--permutations",
        type=int,
    ...
    main(
        args)  # python /Users/andrasjoos/PycharmProjects/ewc_mnist/main_ewc.py --scenario 'pmnist' --ewc_mode 'online' --ewc_lambda 0.4 --lr 1e-4 --hs 256 --epoch 5 --minibatch_size 64 --permutations 4 --cuda 0
